...
export const OWNER_SCREENS: OwnerScreenSpec[] = [
	...
	},
	{
		...
		fields: [
			...
			{ label: "판매중 상품 수", path: "summary.onSaleProductCount" },
		...
		notes: [
			...
			"products 는 **오늘 픽업이 끝나지 않은 것만** 온다(findSellingNowOfStore) — 마감·지난 상품은 빠지고 품절은 남는다. summary.onSaleProductCount 는 이 목록의 개수다",
...

	{
		...
		fields: [
			...
			{ label: "픽업불가(점주 취소)", path: "counts.canceledByOwner" },
			{ label: "찜 취소(손님 취소)", path: "counts.canceledByUser" },
			{ label: "만료된 찜(노쇼)", path: "counts.expired" },
		...
		notes: [
			...
			"칩은 픽업 완료(COMPLETED) · 픽업불가(CANCELED_BY_OWNER — 점주가 취소) · 찜 취소(CANCELED_BY_USER — 손님이 취소) · 만료된 찜(EXPIRED — 노쇼)이다. 픽업불가는 EXPIRED 가 아니다",
		],
	},
	{
		id: "O-040",
